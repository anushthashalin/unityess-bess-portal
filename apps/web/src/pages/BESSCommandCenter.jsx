...
import {
  AlertTriangle, CheckCircle2, Clock, Zap, FileText,
  FolderOpen, Users, TrendingUp, ArrowRight, Battery, Activity,
  Calendar, MapPin, AlertCircle, Bot, Sparkles, Calculator,
} from 'lucide-react';
...
// ── Quick BESS Sizer (AI robot component) ──────────────────────────────────
function QuickSizer() {
  ...
  const runSizing = async () => {
    ...
    let nominal_kwh = 0, nominal_kw = 0, annual_savings = 0, dispatch_yr = 0;
    ...
    if (useCase === 'dg') {
      ...
      nominal_kw   = load;
      dispatch_yr  = load * hrs * days;
      annual_savings = load * hrs * eff * fuel * days;
    } else {
    ...
    // find best unit (prefer 261kWh cabinet or first available)
    ...
    if (unitList.length > 0) {
      ...
      } catch (_) { /* non-blocking */ } finally { setAiLoading(false); }
    }
  };
  ...
  return (
    <Card className="border-orange-200 bg-gradient-to-br from-orange-50/60 to-white">
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 rounded-lg border border-orange-200 bg-white p-0.5">
            <button
              onClick={() => { setUseCase('dg'); setResult(null); setAiNote(null); }}
            ...
            <button
              onClick={() => { setUseCase('tod'); setResult(null); setAiNote(null); }}
      ...
      <CardContent className="space-y-4">
        ...
        <Button
          className="bg-orange-500 hover:bg-orange-600 text-white font-bold h-8 px-5 text-xs gap-2"
          onClick={runSizing}
          disabled={running}
        >
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`inline-flex items-center border rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide ${cls}`}>
      {status.replace(/_/g, ' ')}
    </span>
  );
}
...
function KpiCard({ label, value, sub, icon: Icon, accent, onClick }) {
  return (
    <Card
      className={`cursor-pointer hover:shadow-md transition-all ${accent ? 'border-orange-500/40 bg-orange-50' : ''}`}
      onClick={onClick}
    >
...
export default function BESSCommandCenter() {
  const navigate = useNavigate();

  const { proposals, projects, clients, sites } = useApiMulti({
  ...
  const loading = proposals?.loading || projects?.loading || clients?.loading || sites?.loading;
  ...
  // ── Action items ────────────────────────────────────────────────────────
  // Proposals expiring within 30 days (sent status)
  const expiringProposals = allProposals.filter(p => {
    if (p.status !== 'sent' || !p.validity_days) return false;
    const created = new Date(p.created_at).getTime();
    const expiry  = created + p.validity_days * 86400000;
    return expiry > now && expiry < now + 30 * 86400000;
  });

  // Draft proposals (not yet sent)
  const draftProposals = allProposals.filter(p => p.status === 'draft');

  // Projects stalled at lead/proposal stage
  const stalledProjects = allProjects.filter(p =>
    ['lead', 'proposal', 'negotiation'].includes(p.status)
  );

  // Active / in-progress projects
  const activeProjects = allProjects.filter(p =>
    ['installation', 'commissioned', 'active', 'po_received'].includes(p.status)
  );

  // Sites with no proposals
  const siteIdsWithProposals = new Set(allProposals.map(p => p.site_id).filter(Boolean));
  ...
  const pipelineValue = allProposals
    ...
    .reduce((s, p) => s + Number(p.capex_ex_gst ?? 0), 0);

  const totalAnnualSavings = allProposals
    .filter(p => p.status === 'won' || p.status === 'active')
    .reduce((s, p) => s + Number(p.annual_savings ?? 0), 0);
  ...
  return (
    <div className="flex flex-col gap-6 pb-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-foreground">BESS Command Center</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            {new Date().toLocaleDateString('en-IN', { weekday:'long', day:'numeric', month:'long', year:'numeric' })}
          </p>
        </div>
        {urgentCount > 0 && (
          <Badge className="bg-red-500 text-white text-xs font-bold px-3 py-1.5 gap-1.5">
            <AlertCircle className="w-3.5 h-3.5" /> {urgentCount} item{urgentCount > 1 ? 's' : ''} need attention
          </Badge>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard label="Pipeline Value"   value={inr(pipelineValue)}      sub="Active proposals"        icon={TrendingUp} accent onClick={() => navigate('/proposals')} />
        <KpiCard label="Active Projects"  value={activeProjects.length}    sub="In-progress"             icon={FolderOpen}       onClick={() => navigate('/projects')} />
        <KpiCard label="Total Clients"    value={allClients.length}        sub="Registered"              icon={Users}            onClick={() => navigate('/clients')} />
        <KpiCard label="Annual Savings"   value={inr(totalAnnualSavings)}  sub="Won + commissioned"      icon={Zap}              onClick={() => navigate('/proposals')} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        {/* Expiring proposals */}
        <Card className={expiringProposals.length > 0 ? 'border-amber-300' : ''}>
              ...
              <Table>
                ...
                <TableBody>
                  {expiringProposals.map(p => {
                    ...
                    return (
                      <TableRow key={p.id} className="cursor-pointer hover:bg-amber-50/50" onClick={() => navigate('/proposals')}>
        ...
        {/* Draft proposals */}
        <Card className={draftProposals.length > 0 ? 'border-orange-300' : ''}>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base flex items-center gap-2">
              ...
              <Table>
                ...
                <TableBody>
                  {draftProposals.slice(0, 5).map(p => (
                    <TableRow key={p.id} className="cursor-pointer hover:bg-orange-50/50" onClick={() => navigate('/proposals')}>
                      <TableCell className="font-mono text-xs font-bold text-orange-500">{p.proposal_number}</TableCell>
                      <TableCell className="font-semibold text-sm">{p.company_name}</TableCell>
                      <TableCell className="text-sm font-bold">{inr(p.capex_ex_gst)}</TableCell>
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" className="text-xs text-orange-500 h-7" onClick={() => navigate('/projects')}>
              ...
              <Table>
                ...
                <TableBody>
                  {activeProjects.slice(0, 5).map(p => (
                    <TableRow key={p.id} className="cursor-pointer hover:bg-muted/40" onClick={() => navigate('/projects')}>
                      <TableCell className="font-bold text-sm">{p.project_name}</TableCell>
                      ...
                      <TableCell className="text-sm font-bold text-orange-500">
                        {p.po_value_inr ? inr(p.po_value_inr) : '—'}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
        ...
        {/* Stalled pipeline + sites needing proposal */}
        <div className="flex flex-col gap-4">
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>Projects at lead / proposal / negotiation with no PO</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              ...
              {sitesNeedingProposal.length === 0 ? (
                ...
                <div className="divide-y">
                  {sitesNeedingProposal.slice(0, 4).map(s => (
                    <div key={s.id} className="flex items-center justify-between px-6 py-3 hover:bg-muted/30 cursor-pointer" onClick={() => navigate('/bess-config')}>
                      <div>
